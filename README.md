...
**So the loop is the answer for now, and it is a good one.** You open one agent per role, each runs the five-line loop above, and they self-serialise through `ticketorder.md` — sleeping 300 seconds when blocked, exiting the moment nothing is theirs. No dispatcher, no spawn permissions, no extra tokens. In practice this gets you most of the value at none of the cost.
