...
- The **active project root** is resolved by the `init` parser in `Agent role.md` (e.g., `project1/`, `project3-folder/`, etc.).
...
- **Meta location**: `<SYMPHONY_ROOT>\` contains cross-project profiles, the `.agent_profiles/` folder, `Agent role.md`, and skills.
...
1. Read this file: `<SYMPHONY_ROOT>\.agent_profiles\architect_profile.md`
2. Read the global rules: `<SYMPHONY_ROOT>\skills\global-skill\SKILL.md`
3. Read the protocol: `<SYMPHONY_ROOT>\skills\agent-symphony\SKILL.md`
4. Read the ticket conventions: `<SYMPHONY_ROOT>\skills\ticket-management\SKILL.md`
5. Read the project technical conventions: `<project-folder>\SKILL.md` (resolved by init parser)
6. **[IF YOUR PROJECT HAS A SPEC]** Read `<project-folder>\TECHNICAL_SPECIFICATION.md` — this is the canonical source of truth for all architectural decisions (schema, recurrence rules, OccurrenceEngine, duplicate detection, privacy model, alert engine). All tickets and decisions must align with this UTS.
...
Use the project's naming convention (e.g., `[APPROVED]_P1-XXX_...` for Project1, `[APPROVED]_P3-XXX_...` for Project3). The project prefix is defined in the project's `SKILL.md` or `MEMORY.md`.
...
1. `[HOLD]` / `[DEFER]` lines — intentionally parked; the Orchestrator skips them.
2. In-flight lines — `[IN_PROGRESS]`, or an open `.claims/` marker.
3. **A line carrying a role suffix without its own `:DONE`** — e.g. `WD-279-Dev:DONE:SRTL` means Dev finished *and requested SRTL review*. The `:DONE` belongs to Dev; the `:SRTL` request is still open. **Such a line is NOT terminal.** Confirm by finding a matching `WD-279-SRTL:DONE` line elsewhere in the file; if there is none, the gate is open.
