...
New role (Security/Perf/Docs/Release/Deploy) = (1) Role Registry row in `Agent role.md`, (2) one gate row here (input status → output status), (3) a ring line in `taskagent.md`. The Architect then routes to it with ordinary `<ticket>-<newrole>` lines. Nothing structural changes. Launcher is the release-role instance of this extension; direct human invocation remains interactive and does not require a route line.
