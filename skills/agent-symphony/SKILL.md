...
```
.\rtest.bat --tests "com.example...<DevTicketClassName>"
```
...
For each ticket N in the batch:
  ...
  Dev:  targeted rtest:  .\rtest.bat --tests "com.example...<DevTicketClassName>"  → green
...
For **Project1**: run `rtest --full-cold` first to confirm green — this is the per-batch regression backstop (`clean` + `--no-build-cache`); **only if it is fully green**, run `./gradlew.bat clean assembleDebug --console=plain`, then copy `app\build\outputs\apk\debug\project1-v2.apk` to `Project1-debug.apk` in the project root. Report the file size. If `rtest --full-cold` is not green, do NOT build the artifact — bisect to the culprit ticket and escalate the regression.
test.bat`For **Project1**: run `rtest --full-cold` first to confirm green — this is the per-batch regression backstop (`clean` + `--no-build-cache`); **only if it is fully green**, run `./gradlew.bat clean assembleDebug --console=plain`, then copy `app\build\outputs\apk\debug\project1-v2.apk` to `Project1-debug.apk` in the project root. Report the file size. If `rtest --full-cold` is not green, do NOT build the artifact — bisect to the culprit ticket and escalate the regression.
...
## Project1 — Architect Role (Primary Flow)

For the Project1 project the active lifecycle is the 3-agent Android flow.

**Architect responsibilities**:
- Act as gatekeeper. Analyze user requests for root cause and alignment with architecture + the core philosophy ("Project1 is a memory attached to one or more dates...").
- On every init: re-read the philosophy + invariants from `project1/MEMORY.md`.
...
## Content Project Variant — Project2 Symphony

For content-based projects (e.g., Project2), the pipeline uses 5 agents split across two completely separate lifecycles: **Content Creation** and *
---
...
- Verify your current directory before EVERY script or bulk file operation. The `app/app/` nested-duplicate directory in project1 was created by an agent running with the wrong cwd — never create paths like that; if your op would create a directory that mirrors an existing tree, your cwd is wrong.
...
`app/src/test/java/com/project1/locks/<Area>BehaviorLocksTest.kt` (e.g. `HomeBehaviorLocksTest`, `CalendarBehaviorLocksTest`, `ImportBehaviorLocksTest`). Rules:
