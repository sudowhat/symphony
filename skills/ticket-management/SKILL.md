...
**Preferred / Active usage (Project1):**
...
When creating for Project1, use the `[STATUS]_WD-XXX_...` style and place the file in `project1/tickets/`. Use `[APPROVED]` for a ticket whose Solution Approach + QA instructions are complete and ready for the active batch; use `[DRAFT]` when the design is still being worked out (investigation pending, a decision awaited from the user, or sections not yet finalized) — promote to `[APPROVED]` once finalized.
...
1. **Exact coordinates, never descriptions.** `app/src/main/java/com/project1/ui/MainActivity.kt` → function `fetchDates()` → the `submitList` callback. Not "the home list code".
